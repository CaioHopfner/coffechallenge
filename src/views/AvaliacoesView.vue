...
const mediaCalculada = computed(() => {
  ...
  const soma = (f.aroma || 0) + (f.docura || 0) + (f.acidez || 0) + (f.corpo || 0) + (f.finalizacao || 0)
...
function salvar() {
  ...
  alert(`Avaliação salva: ${cafeSelecionado.value.nome} - Nota: ${mediaCalculada.value}`);
}
</script>

<template>
  <main>
    <h1>Nova Avaliaçao</h1>
    <section class="forms">
      <div>
      ...
      <ul class="lista-dupla">
        ...
        <li>
          ...
          <input id="finalizacao" type="number" v-model.number="formulario.finalizacao" min="0" max="10" />
        </li>
        <li class="especial"><p>{{ mediaCalculada }}</p></li>
      </ul>
    </section>
